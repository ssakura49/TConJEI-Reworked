...
public abstract class AbstractMaterialStatsCategory implements IRecipeCategory<MaterialStatsWrapper> {
    ...
    public RecipeType<MaterialStatsWrapper> recipeType;
    public IDrawable background, icon;
    public List<MaterialStatsId> statsIds;
    @Nullable
    ...
    public void draw(MaterialStatsWrapper wrapper, IRecipeSlotsView recipeSlotsView, GuiGraphics gui, double mouseX, double mouseY) {
        ...
        // Stats — each part's traits (modifiers) are drawn right-aligned on the part's own name line
        for (IMaterialStats stats : statsList) {
            drawTraits(gui, wrapper.getTraits(stats.getIdentifier()), lineNumber);
    ...
    public void getTooltip(ITooltipBuilder tooltips, MaterialStatsWrapper wrapper, IRecipeSlotsView recipeSlotsView, double mouseX, double mouseY) {
        ...
        for (MaterialStatsId statsId : statsIds) {
            ...
            // traits sit on the same line as the part name (right-aligned), matching draw()
            if (addTraitTooltip(tooltips, wrapper.getTraits(stats.getIdentifier()), mouseX, mouseY, lineNumber)) return;
            lineNumber++; // part name line
    ...
    public final void drawString(GuiGraphics gui, String string, int x, float lineNumber, int color, boolean shadow) {
        final int y = (int) (lineNumber * LINE_HEIGHT);
        gui.drawString(FONT, string, x, y, color, shadow);
    }

    public final void drawComponent(GuiGraphics gui, Component component, int x, float lineNumber, int color, boolean shadow) {
        final int y = (int) (lineNumber * LINE_HEIGHT);
        gui.drawString(FONT, component, x, y, color, shadow);
    }

    public final void drawStatComponent(GuiGraphics gui, Component component, float lineNumber) {
    ...
    public final void drawComponentShadowCentered(GuiGraphics gui, Component component, float lineNumber, int color) {
        drawComponent(gui, component, (WIDTH - FONT.width(component)) / 2, lineNumber, color, true);
    }

    public final void drawTraits(GuiGraphics gui, List<ModifierEntry> traits, float lineNumber) {
        for (ModifierEntry trait : traits) {
            final Component component = trait.getDisplayName().plainCopy();
            Style style = trait.getDisplayName().getStyle();
            TextColor textColor = style.getColor();
            final int color = textColor != null ? textColor.getValue() : 0xFFFFFF;
    ...
    public final boolean addStatTooltip(ITooltipBuilder tooltips, IMaterialStats stats, int i, double mouseX, double mouseY, float lineNumber) {
        return addStatTooltip(tooltips, stats, i, 0, mouseX, mouseY, lineNumber);
    }

    public final boolean addStatTooltip(ITooltipBuilder tooltips, IMaterialStats stats, int i, int x, double mouseX, double mouseY, float lineNumber) {
